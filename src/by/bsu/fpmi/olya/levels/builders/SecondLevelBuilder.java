...
public class SecondLevelBuilder extends LevelBuilder{
    ...
    public Level build(){
        ...
        try {
            ComplexLandscape home = getHome();
            initializer.addStructure(home, initializer.getFinishPosition(),
                    GameConstants.SCREEN_HEIGHT - home.getHeight() - FLOOR);
        } catch (LevelException e){
            System.err.printf(e.getMessage());
        }

        Landscape block = getBlock();
        block.setHeight(block.getHeight() - FLOOR);
        initializer.addStructure(getBlock(), initializer.getBlockPosition(), 0);
